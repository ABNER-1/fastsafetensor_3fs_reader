...
def worker_process_template(
    ...
    try:
        ...
        if verbose:
            print(
                f"[Rank {rank}] Will read {len(file_paths)} files x {num_iterations} iterations"
            )
        ...
        for iteration in range(num_iterations):
            ...
            for file_idx, file_path in enumerate(file_paths):
                ...
                if verbose:
                    ...
                    print(
                        f"[Rank {rank}] Iter {iteration+1}/{num_iterations}, "
            ...
            if verbose:
                print(
                    f"[Rank {rank}] Iteration {iteration+1}/{num_iterations} completed in {iteration_time:.2f}s"
                )
        ...
        if verbose:
            print(
                f"[Rank {rank}] Completed all iterations: {avg_throughput_gbps:.2f} GB/s average"
            )
        ...
        result = {"rank": rank, "success": False, "error": str(e)}
        results_queue.put(result)
...
def run_concurrent_test(
    ...
    if verbose:
        ...
        print(
            f"Test mode: {'DOWNLOAD ONLY (no GPU copy)' if download_only else 'FULL (download + GPU copy)'}"
        )
        print(f"\nFile assignment (stride={actual_num_processes}):")
        for rank, rank_files in enumerate(process_files):
            file_indices = [i for i in range(rank, len(files), actual_num_processes)][
                : len(rank_files)
            ]
...
def print_test_results(
    results: List[Dict], num_iterations: int, total_files: int, verbose: bool = True
):
    ...
    if successful_results:
        print(
            f"{'Rank':<6} {'Files':<8} {'Iters':<8} {'Total(GB)':<12} {'Time(s)':<10} {'Throughput':<20}"
        )
        print(f"{'-'*80}")
        ...
        for r in successful_results:
            ...
            print(
                f"{r['rank']:<6} {r['num_files']:<8} {r['num_iterations']:<8} {total_gb:<12.2f} "
        ...
        print(
            f"  Aggregate throughput: {aggregate_throughput_mbps:.2f} MB/s ({aggregate_throughput_gbps:.2f} GB/s)"
        )
        print(
            f"  Theoretical max (sum of all): {theoretical_max_mbps:.2f} MB/s ({theoretical_max_gbps:.2f} GB/s)"
        )
        print(
            f"  Efficiency: {(aggregate_throughput_mbps / theoretical_max_mbps * 100):.1f}%"
        )

        avg_throughput_mbps = sum(
            r["avg_throughput_mbps"] for r in successful_results
