        ...
        for line in f:
            ...
            if match:
                ...
                if timestamp_match:
                    ...
                    events.append(
                        TimelineEvent(timestamp, event_type, batch_id, thread, details)
                    )
...
def analyze_concurrency(events: List[TimelineEvent]) -> Dict:
    ...
    return {
        ...
        "concurrency_ratio": (
            overlap_time / max(total_producer_time, total_consumer_time)
            if max(total_producer_time, total_consumer_time) > 0
            else 0
        ),
    }
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python analyze_queue_timeline.py <log_file_queue_0> <log_file_queue_neg1>"
        )
    ...
    if log_file_neg1:
        ...
        visualize_timeline(
            events_neg1, queue_size=-1, output_file="timeline_queue_neg1.png"
        )

        analysis_neg1 = analyze_concurrency(events_neg1)
        print(f"\nqueue_size=-1 concurrency analysis:")
        print(
            f"  Producer total time: {analysis_neg1.get('total_producer_time', 0):.2f}s"
        )
        print(
            f"  Consumer total time: {analysis_neg1.get('total_consumer_time', 0):.2f}s"
        )
        ...
        print(
            f"  Difference: {(analysis_0.get('concurrency_ratio', 0) - analysis_neg1.get('concurrency_ratio', 0)):.1%}"
        )
