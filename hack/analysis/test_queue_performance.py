...
class PerformanceMetrics:
    ...
    def add_batch(self, batch_id: int, metrics: Dict):
        ...
        self.batch_metrics.append({"batch_id": batch_id, **metrics})
    ...
    def summary(self) -> Dict:
        ...
        total_add_filenames = sum(
            b.get("add_filenames_time", 0) for b in self.batch_metrics
        )
        ...
        return {
            ...
            "avg_time_per_tensor": (
                self.total_time / self.tensor_count if self.tensor_count > 0 else 0
            ),
            ...
            "avg_add_filenames_per_batch": total_add_filenames
...
def test_queue_performance(
    test_files: List[str], queue_size: int, test_name: str
) -> PerformanceMetrics:
    print(f"\n{'='*80}")
...
def compare_results(metrics_0: PerformanceMetrics, metrics_neg1: PerformanceMetrics):
    ...
    print(f"{'-'*80}")
    print(
        f"{'Total time (s)':<30} {time_0:>15.2f} {time_neg1:>15.2f} {(time_neg1/time_0-1)*100:>14.1f}%"
    )
    print(
        f"{'Tensor count':<30} {metrics_0.tensor_count:>15} {metrics_neg1.tensor_count:>15} {'-':>15}"
    )
    ...
    if metrics_0.tensor_count > 0:
        ...
        print(
            f"{'Avg per tensor (ms)':<30} {avg_0:>15.2f} {avg_neg1:>15.2f} {(avg_neg1/avg_0-1)*100:>14.1f}%"
        )
    ...
    if time_neg1 > time_0:
        ...
        print(f"\nReason:")
        print(
            f"  - queue_size=-1: producer must wait for consumer to finish (get_tensor + close)"
        )
        print(
            f"  - queue_size=0:  producer can proceed once consumer dequeues the batch"
        )
        print(f"  - Result: queue_size=0 achieves better pipeline parallelism")
    else:
        print(
            f"queue_size=0 is {(time_0/time_neg1-1)*100:.1f}% slower than queue_size=-1"
        )
...
Examples:
  ...
  python test_queue_performance.py
    ...
    parser.add_argument(
        "files", nargs="*", help="safetensors file paths (glob patterns supported)"
    )
    ...
    try:
        ...
        metrics_0 = test_queue_performance(
            test_files, queue_size=0, test_name="Queue Size = 0 (high-concurrency mode)"
        )
