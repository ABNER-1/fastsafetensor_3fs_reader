...
class NCCLBroadcastBenchmark:
    ...
    def __init__(self, num_rounds: int = 21, warmup_rounds: int = 1):
        ...
        if self.rank == 0:
            print(
                f"[Rank {self.rank}] Generated {format_size(self.data.numel() * 4)} random data"
            )
    ...
    def run_benchmark(self, block_size: int) -> Dict:
        ...
        for round_idx in range(self.num_rounds):
            round_time = self._run_single_round(
                block_size, block_elements, blocks_per_gpu
            )
        ...
        if self.rank == 0:
            results = self._compute_statistics(
                round_times, block_size, total_broadcasts
            )
            return results
    ...
    def _run_single_round(
        self, block_size: int, block_elements: int, blocks_per_gpu: int
    ) -> float:
        ...
        for gpu_id in range(self.world_size):
            for block_id in range(blocks_per_gpu):
                if self.rank == gpu_id:
                    ...
                    block_data = torch.empty(
                        block_elements, dtype=torch.float32, device=self.device
                    )
    ...
    def _compute_statistics(
        self, round_times: List[float], block_size: int, total_broadcasts: int
    ) -> Dict:
        import statistics
    ...
    def print_summary(self, all_results: List[Dict]):
        ...
        print(
            f"│ {'Block':<8} │ {'Broadcasts':<10} │ {'Avg Round':<11} │ {'Total BW':<12} │ {'Single':<8} │"
        )
        print(
            f"│ {'Size':<8} │ {'per Round':<10} │ {'Time':<11} │ {'(GB/s)':<12} │ {'BW':<8} │"
        )
        print(f"├{'─'*10}┼{'─'*12}┼{'─'*13}┼{'─'*14}┼{'─'*10}┤")
        ...
        for result in all_results:
            ...
            print(
                f"│ {block_size_str:<8} │ {broadcasts:<10} │ {avg_time:>9.1f} ms │ "
    ...
    def save_results(
        self, all_results: List[Dict], output_file: str = "nccl_broadcast_results.csv"
    ):
        if self.rank != 0:
            return
        ...
        with open(output_file, "w", newline="") as f:
            ...
            for result in all_results:
                ...
                for round_idx, round_time in enumerate(result["round_times"]):
                    bandwidth = (
                        (self.data_per_gpu * self.world_size) / round_time / (1024**3)
                    )
                    writer.writerow(
                        [
                            block_size,
...
def main():
    ...
    parser.add_argument(
        "--rounds", type=int, default=21, help="Number of test rounds (default: 21)"
    )
    parser.add_argument(
        "--warmup", type=int, default=1, help="Number of warmup rounds (default: 1)"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="nccl_broadcast_results.csv",
        help="Output CSV file (default: nccl_broadcast_results.csv)",
    )

    args = parser.parse_args()

    benchmark = NCCLBroadcastBenchmark(
        num_rounds=args.rounds, warmup_rounds=args.warmup
    )
